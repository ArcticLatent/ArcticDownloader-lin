...
impl TrayIcon {
    pub fn new(id: TrayIconId, attrs: TrayIconAttributes) -> crate::Result<Self> {
        let indicator = Indicator::new(
            &format!("tray-icon tray app {}", id.as_ref()),
            "",
        ...
        if let Some(menu) = &attrs.menu {
            indicator.set_menu(Some(&menu.gtk_context_menu()));
        }
    ...
    pub fn set_menu(&mut self, menu: Option<Box<dyn crate::menu::ContextMenu>>) {
        if let Some(menu) = &menu {
            self.indicator.set_menu(Some(&menu.gtk_context_menu()));
        }
        self.menu = menu;
    }
    ...
    pub fn app_indicator(&self) -> &Indicator {
        &self.indicator
    }
}
